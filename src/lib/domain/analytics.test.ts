...
const behaviors: Behavior[] = [
  { id: "participation", name: "Participation", color: "#000", mode: "tally", position: 0 },
...
const run = (over: Partial<Parameters<typeof aggregate>[0]> = {}) => aggregate({
  taps, students, classes, now: NOW, window: "all",
...
test("sorting by name is alphabetical and reversible", () => {
  ...
  expect(up.map((row) => row.studentName)).toEqual(["Kai B", "Maya C", "Sam A"]);
  expect(down.map((row) => row.studentName)).toEqual(["Sam A", "Maya C", "Kai B"]);
});
...
test("a name containing a comma survives the round trip", () => {
  const rows = [{
    studentId: "x", studentName: 'Chen, Maya', className: "Period 2",
    counts: { participation: 1, redirect: 0 }, total: 1,
  }];
