...
import {
  ANCHOR_MIN, GRID, SEAT_SIZE, fitScale, layoutExtent, nextAnchorSpot, nextSeatSpot, placeSeat,
  roomBounds, sizeAnchor, snap,
} from "./seating";
import type { Anchor, Seat } from "./types";

const seat = (id: string, x: number, y: number): Seat => ({ id, x, y });
const anchor = (id: string, x: number, y: number, width = 200, height = 80): Anchor =>
  ({ id, x, y, width, height, label: id });
...
test("the room's own box ignores the empty floor around it", () => {
  expect(roomBounds([seat("a", 100, 60), seat("b", 300, 60)]))
...
test("an Anchor is part of the room, so the box and the canvas both hold it", () => {
  expect(roomBounds([seat("a", 100, 100)], [anchor("board", 0, 0, 400, 60)]))
    .toEqual({ x: 0, y: 0, width: 400, height: 100 + SEAT_SIZE });
  expect(layoutExtent([seat("a", 0, 0)], [anchor("board", 0, 3000, 400, 60)]).height)
    .toBeGreaterThan(3000);
});

test("a new Anchor is born below everything already in the room", () => {
  expect(nextAnchorSpot([], [])).toEqual({ x: 0, y: 0 });
  expect(nextAnchorSpot([seat("a", 0, 200)], []).y).toBeGreaterThan(200 + SEAT_SIZE);
});

test("an Anchor resized to nothing keeps enough of itself to read", () => {
  expect(sizeAnchor(5, 5, false)).toEqual({ width: ANCHOR_MIN, height: ANCHOR_MIN });
