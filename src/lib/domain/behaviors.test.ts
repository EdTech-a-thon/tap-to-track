...
import { ABSENT_BEHAVIOR, BEHAVIOR_LIMIT, DEFAULT_BEHAVIORS, absentBehavior, behaviorsFor } from "./behaviors";
...
test("the Absent row is found by its rule, not by its name", () => {
  const all = [make("a", 0), { ...make("absent", 1), name: "Absent", away: true }];
