...
export function Groups() {
  ...
  return <section className="group-settings">
    ...
    <form className="card group-builder" onSubmit={addGroup}><label>New group name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Table 1" /></label><button className="primary" disabled={!name.trim() || adding}>{adding ? "Adding..." : "Add group"}</button>{message && <p className="error" role="alert">{message}</p>}</form>
