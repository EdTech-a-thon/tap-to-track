...
type ReportTab = "insights" | "participation" | "attendance" | "skills";
...
export function Progress() {
  ...
  useEffect(() => {
    if (!classId || range === "custom" && (!from || !to)) return;
    let current = true;
    setLoading(true);
  ...
  }, [classId, from, range, reportQuery, snapshot?.attendance.length, snapshot?.events.length, snapshot?.mastery.length, tag, to]);
  ...
  return <section className="reports-page">
    <div className="page-heading"><div><p className="eyebrow">COUNTS AND EVIDENCE</p><h1>Insights</h1></div><div className="button-row"><label>Scope<select value={scope} onChange={(event) => setScope(event.target.value as "current" | "all")}><option value="current">Current class</option><option value="all">All classes</option></select></label>
      <button className="secondary" onClick={() => { location.href = `/api/classes/${classId}/reports/export/csv?${reportQuery}`; }}>Export CSV</button>
    ...
    <nav className="report-tabs" aria-label="Insights sections">{(["insights", "participation", "attendance", "skills"] as ReportTab[]).map((item) => <button className={tab === item ? "active" : ""} aria-current={tab === item ? "page" : undefined} onClick={() => setTab(item)} key={item}>{item}</button>)}</nav>
    ...
    {!loading && report && <>
      ...
      {tab === "participation" && <Participation report={report} students={students} onSelect={setSelectedId}/>} 
      {tab === "attendance" && <Attendance report={report} students={students} snapshot={snapshot} classId={classId} onRefresh={refresh}/>} 
...
function ParticipationStatus({ student, report }: { student: ReportStudent; report: ClassReport }) { const guidance = participationGuidanceLevel(student.periodsSincePositive ?? student.participationEligiblePeriods, report.classRoom.settings); const status = student.participationEligiblePeriods === 0 ? "No eligible days" : guidance === "check-in" ? "Check in" : guidance === "watch" ? "Watch" : "On track"; return <span className={`report-status ${guidance === "check-in" ? "needs-attention" : guidance === "watch" ? "building" : "regular"}`}>{status}</span>; }
