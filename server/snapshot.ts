...
export function getClassSnapshot(db: AppDatabase, teacherId: string, classId: string) {
  ...
  const masteryEvents = db.prepare("SELECT id, studentId, skillId, previousAchievement, achievement, previousRequiresSupport, requiresSupport, timestamp, periodId, skillLabel, category, note FROM mastery_events WHERE classId = ? AND teacherId = ? ORDER BY timestamp, rowid").all(classId, teacherId) as Row[];
